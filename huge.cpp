...
//------------ Added June 12, 2018 ----------
//Modified from align(), assuming the cluster labels have already been matched
//Same number of clusters in each result, wt[][] is diagonal, no permutation needed
void alignDone(int *cls, int ns, int len, float **wt, int **clsct_pt, float **dc_pt)
{
    int i,j,k,m;
    float *dc, *cpct,*thiswt;
    int **clsarray, *clsct;
    ...
    clsct=(int *)calloc(ns,sizeof(int));//number of clusters in each partition result
    for (i=0;i<ns;i++){
        clsct[i]=0;
        for (j=0;j<len;j++) {if (clsarray[i][j]>clsct[i]) clsct[i]=clsarray[i][j];}
        clsct[i]++;
    ...
    for (i=0;i<ns;i++)
        fprintf(stdout, "%d ",clsct[i]);
    fprintf(stdout, "\n");
    
    //force to have same number of clusters for each sample
    for (i=1;i<ns;i++){
        if (clsct[0]<clsct[i]) {
            fprintf(stderr, "Warning: reference clustering has fewer clusters than result %d, %d<%d\n",
                    i,clsct[0],clsct[i]);
            exit(1);
        }
        else {clsct[i]=clsct[0];}
    }
    
    cpct=(float *)calloc(clsct[0],sizeof(float));
    for (i=0;i<clsct[0]; i++) cpct[i]=0.0;
    for (i=0;i<len;i++) cpct[clsarray[0][i]]+=1.0;
    for (i=0;i<clsct[0]; i++) cpct[i]/=(float)len;
    
    dc=(float *)calloc(ns,sizeof(float));
    dc[0]=0.0; //distance to itself
    m=(ns-1)*clsct[0]*clsct[0];
    ...
    *wt=(float *)calloc(m,sizeof(float));
    for (i=1,m=0;i<ns;i++) {
        ...
        for (j=0;j<clsct[i];j++)
            for (k=0;k<clsct[0];k++) {
                if (j==k) {
                    thiswt[j*clsct[0]+k]=cpct[j]; //diagonal weight matrix
                }
                else {thiswt[j*clsct[0]+k]=0.0;}
            }
        
        dc[i]=0.0;
        if (!VERSION2) {
            for (j=0; j<clsct[0]; j++)
                dc[i]+=(cpct[j]*dist2cls(clsarray[0],clsarray[i],len,j,j));
        } else {//Use the normalized distance with respect to each cluster's size
            for (j=0; j<clsct[0]; j++)
                dc[i]+=(cpct[j]*dist2cls_normalized(clsarray[0],clsarray[i],len,j,j));
        }
...
// [[Rcpp::export]]
List ACPS(IntegerVector x, int nbs, int reference, int aligned)
{
    ...
    if (aligned!=0 && aligned!=1) {
        Rcpp::stop("Wrong aligned value");
    }
    else if (aligned == 0){
        align(cls,nbs,len,&wt,&numcls,&dist,equalcls);
    }
    else{
        alignDone(cls,nbs,len,&wt,&numcls,&dist);
    }
